...
const validationRules = [
    check('name')
        ...
        .isLength({ min: 1, max: 50 }).withMessage('Username must be between 8 and 20 characters'),
    check('password')
        .notEmpty().withMessage('Password is required')
        .isLength({ min: 1 }).withMessage('Password must be larger than 6')
];
...
class UsersController {
    ...
    static async register(req, res) {
        ...
        if (!errors.isEmpty()) {
            ...
            const exists = await UserORM.findOne(
